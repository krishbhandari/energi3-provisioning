...
```
bash -ic "$(wget -4qO- -o- raw.githubusercontent.com/energicryptocurrency/energi3-provisioning/master/scripts/linux/energi3-aarch64-installer.sh)" ; source ~/.bashrc
```
'
######################################################################
...
#  BIN_URL=$( echo "${GITHUB_LATEST}" | jq -r '.assets[].browser_download_url' | grep -v debug | grep -v '.sig' | grep linux )
  ...
  wget -4qo- "${S3URL}/${GIT_LATEST}/energi3-${GIT_LATEST}-linux-${OSARCH}.tgz" --show-progress --progress=bar:force:noscroll 2>&1
  #wget -4qo- "${BIN_URL}" -O "${ENERGI3_EXE}" --show-progress --progress=bar:force:noscroll 2>&1
  sleep 0.3
  
  tar xvfz energi3-${GIT_LATEST}-linux-${OSARCH}.tgz
  sleep 0.3
  
  # Copy latest energi3 and cleanup
  if [[ -x "${ENERGI3_EXE}" ]]
  then
    mv energi3-${GIT_LATEST}-linux-${OSARCH}/bin/energi3 ${BIN_DIR}/.
    rm -rf energi3-${GIT_LATEST}-linux-${OSARCH}
  else
    mv energi3-${GIT_LATEST}-linux-${OSARCH} ${ENERGI3_HOME}
  fi
  rm energi3-${GIT_LATEST}-linux-${OSARCH}.tgz
  
  # Check if software downloaded
  if [ ! -d ${BIN_DIR} ]
  then
    echo "${RED}ERROR: energi3-${GIT_LATEST}-linux-${OSARCH}.tgz did not download${NC}"
    sleep 5
  fi
